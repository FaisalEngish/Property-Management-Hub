...
- **Real-Time Data Updates**: Centralized query key management system (`client/src/lib/queryKeys.ts`) ensures consistent cache invalidation across all modules. Query key hierarchy supports both global and property-specific data views. Invalidation helpers (`invalidateFinanceQueries`, `invalidatePropertyQueries`, `invalidateBookingQueries`) automatically refresh related data across Finance, Properties, and Bookings modules when mutations occur, enabling real-time updates without manual cache management.
