...
export default function MaintenanceLogWarrantyTracker() {
  ...
  const { data: appliancesData } = useQuery({ queryKey: ["/api/property-appliances"] });

  const maintenanceRecords = (tasks || [])
    .filter((task: any) => task.category === "maintenance")
    .slice(0, 10)
    .map((task: any) => {
      const property = (propertiesData || []).find((p: any) => p.id === task.propertyId);
      return {
        id: task.id,
        property: property?.name || `Property ${task.propertyId}`,
        category: task.subcategory || "General",
        description: task.description || task.title,
        date: task.completedAt || task.createdAt,
        technician: task.assignedStaff || "Unassigned",
        cost: task.actualCost || 0,
        status: task.status,
        warrantyExpiry: null,
        nextService: task.scheduledDate,
        priority: task.priority || "medium",
        photos: []
      };
    });

  const warrantyAlerts = (appliancesData || [])
    .filter((a: any) => a.warrantyExpiry)
    .slice(0, 10)
    .map((appliance: any) => {
      const property = (propertiesData || []).find((p: any) => p.id === appliance.propertyId);
      const expiryDate = new Date(appliance.warrantyExpiry);
      const daysUntilExpiry = Math.floor((expiryDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
      return {
        id: appliance.id,
        property: property?.name || `Property ${appliance.propertyId}`,
        item: appliance.name,
        category: appliance.category || "Appliances",
        purchaseDate: appliance.purchaseDate,
        warrantyExpiry: appliance.warrantyExpiry,
        daysUntilExpiry,
        status: daysUntilExpiry < 0 ? "expired" : daysUntilExpiry < 30 ? "warning" : "active",
        vendor: appliance.manufacturer || "Unknown",
        cost: appliance.purchasePrice || 0
      };
    });
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-purple-600">{formatWithConversion(summaryStats.totalCost)}</p>
                  </div>
                  ...
                  <div key={record.id} className="border rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-green-600">{formatWithConversion(record.cost)}</p>
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Vendor: {alert.vendor} • Cost: {formatWithConversion(alert.cost)}
                        </p>
                      </div>
                    </div>
                  ...
                  <div key={prediction.id} className="border rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                      <div>
                        ...
                        <p className="font-medium text-orange-600">{formatWithConversion(prediction.estimatedCost)}</p>
                      </div>
                      <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold">{formatWithConversion(2500)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span>Pool Maintenance</span>
                  <span className="font-bold">{formatWithConversion(4200)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span>Electrical Work</span>
                  <span className="font-bold">{formatWithConversion(6500)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span>Garden & Landscaping</span>
                  <span className="font-bold">{formatWithConversion(3200)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span>Plumbing</span>
                  <span className="font-bold">{formatWithConversion(800)}</span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {maintenanceRecords.filter(r => r.status === "scheduled").slice(0, 4).map((record) => (
                  <div key={record.id} className="flex justify-between">
                    <span className="text-sm">{record.property} - {record.category}</span>
                    <span className="text-sm font-medium">{record.nextService || "TBD"}</span>
                  </div>
                ))}
