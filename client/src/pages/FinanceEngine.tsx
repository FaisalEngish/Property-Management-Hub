...
export default function FinanceEngine() {
  ...
  const { data: properties = [] } = useQuery<any[]>({
    ...
    select: (data) => data.map(p => ({ id: p.id, name: p.name, location: p.location || "Unknown" }))
  });
  ...
  // Owner balances fetched from API - computed from properties and bookings
  const ownerBalances = properties.map((property: any, index: number) => ({
    id: property.id,
    ownerName: property.ownerName || `Owner ${index + 1}`,
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-600">
                      {formatWithConversion(financialSummary.totalRevenue)}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-red-600">
                      {formatWithConversion(financialSummary.totalExpenses)}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-blue-600">
                      {formatWithConversion(financialSummary.netProfit)}
                    </p>
                  </div>
                  ...
                  {revenueBreakdown.map((source, index) => (
                    <div key={index} className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="font-bold">{formatWithConversion(source.amount)}</p>
                ...
                <div className="space-y-3">
                  {expenseCategories.map((category, index) => (
                    <div key={index} className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="font-bold">{formatWithConversion(category.amount)}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-xl font-bold">{formatWithConversion(financialSummary.ownerPayouts)}</p>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-xl font-bold">{formatWithConversion(financialSummary.pendingPayouts)}</p>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-xl font-bold">{formatWithConversion(financialSummary.commissions)}</p>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-xl font-bold">{formatWithConversion(financialSummary.managementFees)}</p>
              </CardContent>
            </Card>
          </div>
        </TabsContent>
                  ...
                  <div className="space-y-2">
                    {revenueBreakdown.map((source, index) => (
                      <div key={index} className="flex justify-between items-center p-2 rounded border">
                        ...
                        <div className="text-right">
                          <div className="font-bold">{formatWithConversion(source.amount)}</div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between p-2 rounded border">
                      ...
                      <span className="font-bold">{formatWithConversion(245000)}</span>
                    </div>
                    <div className="flex justify-between p-2 rounded border">
                      <span>December 2024</span>
                      <span className="font-bold">{formatWithConversion(220000)}</span>
                    </div>
                    <div className="flex justify-between p-2 rounded border">
                      <span>November 2024</span>
                      <span className="font-bold">{formatWithConversion(198000)}</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                {expenseCategories.map((category, index) => (
                  <div key={index} className="border rounded-lg p-4">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        <div className="font-bold">{formatWithConversion(category.amount)}</div>
                    ...
                    <div key={owner.id} className="border rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-bold text-lg">{formatWithConversion(owner.currentBalance)}</p>
                        </div>
                        <div>
                          <p className="text-gray-500">Pending Payout</p>
                          <p className="font-bold text-yellow-600">{formatWithConversion(owner.pendingPayout)}</p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="font-bold text-green-600">{formatWithConversion(owner.totalEarnings)}</p>
                        </div>
                      </div>
                    ...
                    <div key={transaction.id} className="flex items-center justify-between p-3 border rounded">
                      ...
                      <div className="text-right">
                        ...
                        <p className={`font-bold text-lg ${transaction.amount > 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {transaction.amount > 0 ? '+' : ''}{formatWithConversion(Math.abs(transaction.amount))}
                        </p>
                      </div>
                    </div>
                  );
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold">{formatWithConversion(7903)}</span>
                  </div>
                  <div className="flex justify-between">
