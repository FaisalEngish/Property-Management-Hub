...
export default function Bookings() {
  ...
  const searchString = useSearch(); // Get query string from wouter

  // show overlay whenever any mutation is active
  const activeMutations = useIsMutating();
  const isMutating = activeMutations > 0;

  // Get propertyId from URL using wouter's useSearch()
  ...
  // Fetch real data from API - use specific endpoint if propertyId is in URL
  const { data: bookings = [], isLoading: bookingsLoading } = useQuery({
    queryKey: urlPropertyId
      ? queryKeys.bookings.withSource(urlPropertyId)
  ...
  const filteredBookings = (urlPropertyId
    ...
    // Sort chronologically - newest bookings first (by createdAt)
  ...
  // Group bookings by property for calendar view
  const propertiesWithBookings = propertiesArray
    .map((property: any) => ({
      ...property,
  ...
  return (
    <div className="relative container mx-auto min-h-screen bg-background">
      ...
      <div className="p-3 sm:p-6">
              ...
              {urlPropertyId && selectedProperty ? (
                <>
                  ...
                  <p className="text-sm sm:text-base text-muted-foreground">
                    Viewing all bookings for this property
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-sm sm:text-base text-muted-foreground">
                    Manage all bookings, calendars, and property schedules
                  </p>
                </>
              )}
            </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-3 h-auto">
            <TabsTrigger value="bookings" className="gap-1 sm:gap-2 text-xs sm:text-sm px-1 sm:px-3 py-2">
              <List className="w-3 h-3 sm:w-4 sm:h-4" />
              <span className="hidden sm:inline">Booking List</span>
              <span className="sm:hidden">List</span>
            </TabsTrigger>
            <TabsTrigger value="calendar" className="gap-1 sm:gap-2 text-xs sm:text-sm px-1 sm:px-3 py-2">
              <Calendar className="w-3 h-3 sm:w-4 sm:h-4" />
              <span className="hidden sm:inline">Multi-Property Calendar</span>
              <span className="sm:hidden">Calendar</span>
            </TabsTrigger>
            <TabsTrigger value="live" className="gap-1 sm:gap-2 text-xs sm:text-sm px-1 sm:px-3 py-2">
          ...
          <TabsContent value="bookings" className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold">
                Individual Bookings ({filteredBookings.length})
              </h3>
              <Badge variant="outline">
                Total Revenue: ฿
                {filteredBookings
                    ...
                    >
                      <CardContent className="p-3 sm:p-4">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                          ...
                          {/* Property details - hidden on mobile */}
                          <div className="hidden lg:flex items-center gap-4 text-sm">
                            <div className="flex items-center gap-1">
                              <MapPin className="w-4 h-4" />
                              <span className="truncate max-w-[150px]">{property?.address || "N/A"}</span>
                            </div>
                            {property?.bedrooms && (
                              <div className="flex items-center gap-1">
                                <Bed className="w-4 h-4" />
                          ...
                          {/* Mobile: show dates, amount, and status in a row */}
                          <div className="flex flex-wrap sm:flex-nowrap items-center justify-between sm:justify-end gap-2 sm:gap-4">
                            ...
                            <div className="text-left sm:text-right order-1 sm:order-2">
                              ...
                              <div className="flex items-center gap-1 sm:gap-2 sm:justify-end mt-1">
          ...
          <TabsContent value="calendar" className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold">
                Multi-Property Calendar ({propertiesWithBookings.length}{" "}
                properties)
              </h3>
              <Badge variant="outline">
                Total Bookings:{" "}
                {propertiesWithBookings.reduce(
                  (sum: number, p: any) => sum + p.bookings.length,
                  0,
                )}
              </Badge>
            </div>
