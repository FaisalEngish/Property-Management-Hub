...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (outstandingPayments.length > 0) {
      ...
      toast({
        ...
        description: `${outstandingPayments.length} customer${outstandingPayments.length > 1 ? "s have" : " has"} outstanding balances totaling ${formatWithConversion(totalOutstanding)}`,
        variant: "default",
      });
    }
  ...
  return (
    <div className="container mx-auto bg-background w-full overflow-x-hidden">
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 overflow-auto p-3 sm:p-6 overflow-x-hidden">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 lg:gap-6 mb-4 sm:mb-8">
            <StatsCard
              ...
              value={
                // Use stats API count when available and no filters applied, otherwise use filtered count
                Object.keys(activeFilters).length === 0 || 
                (!activeFilters.searchText && !activeFilters.area && !activeFilters.bedroomCount)
                  ? (isStatsLoading ? "-" : (stats?.totalProperties ?? filteredProperties.length))
            ...
            <StatsCard
              ...
              value={
                isPropertiesLoading 
                  ? "-" 
                  : filteredProperties.filter((p: any) => p.status === "active").length
              }
              icon={Home}
              color="success"
            />
          ...
          >
            <TabsList className="grid w-full grid-cols-4 h-auto">
              ...
              <TabsTrigger value="properties" className="text-xs sm:text-sm px-1 sm:px-3 py-2">
                <span className="hidden sm:inline">Properties</span>
                <span className="sm:hidden">Props</span>
              </TabsTrigger>
              <TabsTrigger value="tasks" className="text-xs sm:text-sm px-1 sm:px-3 py-2">Tasks</TabsTrigger>
              <TabsTrigger value="bookings" className="text-xs sm:text-sm px-1 sm:px-3 py-2">
                          ...
                          >
                            ...
                            <div className="text-right">
                              <Badge variant="destructive">
                                {formatWithConversion(amountDue)} due
                              </Badge>
