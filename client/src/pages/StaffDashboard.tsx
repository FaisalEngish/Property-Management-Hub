...
export default function StaffDashboard() {
  ...
  const { data: expensesData } = useQuery({ queryKey: ["/api/staff-expenses"] });
  const { data: collectionsData } = useQuery({ queryKey: ["/api/staff-cash-collection"] });

  const staffWalletBalance = walletData?.currentBalance || 0;
  const basePettyCash = walletData?.basePettyCash || 0;
  const pendingExpenses = (expensesData || []).filter((e: any) => e.status === "pending_review").length;
  const pendingCollections = (collectionsData || []).filter((c: any) => c.status === "pending").length;
  ...
  const quickActions = [
    ...
    },
    {
      ...
      count: formatWithConversion(staffWalletBalance),
      icon: Wallet,
      href: "/staff-wallet-petty-cash",
      color: "green",
      description: `Base: ${formatWithConversion(basePettyCash)}`
    },
    {
    ...
    },
    {
      ...
      count: `${pendingCollections} pending`,
      icon: DollarSign,
  ...
  const todaysTasks = tasks?.slice(0, 5).map((task: any) => ({
    id: task.id,
    title: task.title,
    property: task.propertyName || `Property ${task.propertyId}`,
