...
export default function DailyOperationsDashboard() {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(`/api/daily-operations/summary/${selectedDate}`);
  ...
  >({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/daily-operations/staff/${selectedDate}`);
  ...
  >({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/daily-operations/properties/${selectedDate}`);
  ...
  const { data: tasks, isLoading: tasksLoading } = useQuery<OperationsTask[]>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/daily-operations/tasks/${selectedDate}`);
  ...
  const filterByProperty = (items: any[] | undefined) => {
    if (!items) return [];
    if (selectedPropertyId === "all") return items;
    return items.filter((item: any) => 
      String(item.propertyId) === selectedPropertyId
    );
  };

  // Apply property filter to data
  const filteredPropertyOps = filterByProperty(propertyOps);
  const filteredTasks = filterByProperty(tasks);
  const filteredStaffAssignments = selectedPropertyId === "all" 
    ? staffAssignments 
    : staffAssignments?.filter((staff: any) => 
        staff.tasks?.some((task: any) => String(task.propertyId) === selectedPropertyId)
      );
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {/* Main Container with proper spacing from sidebar */}
      <div className="max-w-7xl mx-auto px-6 lg:px-10 space-y-8 pt-6">
        {/* Header */}
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 pt-8">
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold flex items-center gap-2 whitespace-nowrap">
        ...
        <TabsContent value="staff" className="space-y-4">
          <Card>
            ...
            <CardContent>
              ...
              ) : filteredStaffAssignments && filteredStaffAssignments.length > 0 ? (
                <div className="space-y-4">
                  {filteredStaffAssignments.map((assignment:any) => (
                    <div key={assignment.id} className="border rounded-lg p-4" data-testid={`card-staff-${assignment.staffId}`}>
        ...
        <TabsContent value="properties" className="space-y-4">
          <Card>
            ...
            <CardContent>
              ...
              ) : filteredPropertyOps && filteredPropertyOps.length > 0 ? (
                <div className="space-y-4">
                  {filteredPropertyOps.map((property) => (
                    <div key={property.id} className="border rounded-lg p-4" data-testid={`card-property-${property.propertyId}`}>
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h4 className="font-semibold">
                ...
                <div className="space-y-4">
                  {filteredTasks.map((task) => (
                    <div 
                      ...
                      onClick={() => setLocation(`/tasks?taskId=${task.id}`)}
