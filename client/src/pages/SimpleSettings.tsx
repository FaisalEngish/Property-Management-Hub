...
import { invalidateSystemSettingsQueries } from "@/lib/queryKeys";
...
export default function SimpleSettings() {
  ...
  const { data: fetchedSystemSettings } = useQuery({
  ...
  useEffect(() => {
    if (fetchedSystemSettings) {
      setSystemSettings({
        organizationName: fetchedSystemSettings.companyName || 'HostPilotPro',
        currency: fetchedSystemSettings.displayCurrency || fetchedSystemSettings.defaultCurrency || 'THB',
        dateFormat: fetchedSystemSettings.dateFormat || 'DD/MM/YYYY',
      });
    }
  ...
  const updateSystemSettingsMutation = useMutation({
    mutationFn: async (data: { displayCurrency?: string; defaultCurrency?: string; dateFormat?: string; companyName?: string }) => {
      return await apiRequest('PUT', '/api/system-settings', data);
    },
    onSuccess: () => {
      // Use the helper to clear fastCache AND invalidate queries
      invalidateSystemSettingsQueries(queryClient);
      toast({
        title: "Settings Updated",
        description: "Your settings have been updated. All amounts will now be converted to the new currency.",
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const handleSystemSettingChange = (field: string, value: string) => {
    ...
    const fieldMapping: Record<string, string> = {
      'currency': 'displayCurrency', // Use displayCurrency for conversion
    ...
    // Save to database
    updateSystemSettingsMutation.mutate({ [apiField]: value });
  };
