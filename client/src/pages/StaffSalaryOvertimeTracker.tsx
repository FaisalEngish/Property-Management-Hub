            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {payrollSummary?.totalPayroll ? formatWithConversion(payrollSummary.totalPayroll) : "$0.00"}
                </div>
                ...
                <div className="space-y-4">
                  {advanceRequests?.map((request: any) => (
                    <div key={request.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div>
                        <p className="font-medium">{formatWithConversion(request.amount)}</p>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    <div className="p-4 border rounded-lg">
                      ...
                      <p className="text-2xl font-bold">{formatWithConversion(payrollSummary.totalFixedSalaries)}</p>
                    </div>
                    <div className="p-4 border rounded-lg">
                      <p className="text-sm text-muted-foreground">Overtime Pay</p>
                      <p className="text-2xl font-bold">{formatWithConversion(payrollSummary.totalOvertimePay)}</p>
                    </div>
                    <div className="p-4 border rounded-lg">
                      <p className="text-sm text-muted-foreground">Emergency Bonuses</p>
                      <p className="text-2xl font-bold">{formatWithConversion(payrollSummary.totalEmergencyBonuses)}</p>
                    </div>
                  </div>
                      ...
                      <div className="space-y-2">
                        {Object.entries(payrollSummary.departmentBreakdown).map(([dept, amount]: [string, any]) => (
                          <div key={dept} className="flex justify-between items-center p-2 border rounded">
                            ...
                            <span className="font-medium">{formatWithConversion(amount)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
