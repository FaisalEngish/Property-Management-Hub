    ...
    <div className="space-y-1">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`font-semibold text-blue-600 dark:text-blue-400 ${sizeClasses[size]}`}>
          {formatWithConversion(guestTotalPrice)}
        </span>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`font-bold ${emphasizePayoutAsRevenue ? 'text-green-700 dark:text-green-400' : 'text-green-600 dark:text-green-400'} ${sizeClasses[size]}`}>
          {formatWithConversion(platformPayout)}
        ...
        <div className="flex items-center justify-between pt-1 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center gap-1">
            <span className="text-xs text-red-600 dark:text-red-400 font-medium">
              {formatWithConversion(commissionAmount)}
            </span>
        ...
        <Tooltip>
          ...
          <TooltipContent className="max-w-xs">
            <div className="space-y-2">
              ...
              <p className="text-xs">
                The guest paid <strong>{formatWithConversion(guestTotalPrice)}</strong> to {platform}, 
                but you only receive <strong>{formatWithConversion(platformPayout)}</strong> after 
        ...
        <TooltipTrigger asChild>
          <div className="text-center cursor-help">
            <div className="text-xs text-gray-500">
              Guest: {formatWithConversion(guestTotalPrice)}
            </div>
            <div className="font-semibold text-green-600">
              Payout: {formatWithConversion(platformPayout)}
            </div>
            {commissionAmount > 0 && (
    ...
    <div className="p-4 border rounded-lg bg-gradient-to-r from-blue-50 to-green-50">
      ...
      <div className="mt-3 p-2 bg-orange-50 rounded border-l-4 border-orange-400">
        <p className="text-xs text-orange-700">
          💡 All commission calculations are based on the payout amount ({formatWithConversion(platformPayout)})
        </p>
      </div>
    </div>
  );
