...
export function MultiPropertyCalendar({
  ...
  console.log("MultiPropertyCalendar received bookings:", bookings);
  console.log("Hostaway bookings count:", bookings.filter(b => b.source === 'HOSTAWAY').length);
    ...
    return optimisticBookings
      ...
      .filter(
        (booking) =>
          ...
          booking.propertyId.toString() === selectedProperty,
      );
  };
  ...
  const handleDrop = (e: React.DragEvent, targetDate: Date) => {
    ...
    if (draggedBooking && targetDate) {
      ...
      if (
        draggedBooking.status === "checked-in" ||
        draggedBooking.status === "completed"
      ) {
        toast({
          title: "❌ Cannot Reschedule",
          description:
            "Bookings that are checked-in or completed cannot be rescheduled.",
          variant: "destructive",
        });
      ...
      if (hasConflict(targetDate, draggedBooking)) {
        toast({
          ...
          description:
            "These dates conflict with an existing booking for this property.",
          variant: "destructive",
        });
        setDraggedBooking(null);
        setDragOverDate(null);
    ...
    return (
      <div className="p-2 space-y-1">
        <div className="font-semibold text-sm">
          {booking.source === 'HOSTAWAY' && <span className="text-[10px] bg-purple-600 text-white px-1 rounded mr-1">H</span>}
          {booking.source === 'LOCAL' && <span className="text-[10px] bg-emerald-600 text-white px-1 rounded mr-1">L</span>}
          {booking.guestName}
        </div>
        ...
        {booking.source === 'HOSTAWAY' && booking.bookingPlatform && (
          <div className="text-xs text-slate-500">Platform: {booking.bookingPlatform}</div>
        )}
      ...
      <CardContent>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-7 gap-2 mb-4">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
              <div
                key={day}
                className="p-2 text-center font-medium text-slate-600 bg-slate-50 rounded"
              >
                {day}
              </div>
            ))}
                        ...
                        <div className="flex items-center justify-between mb-2">
                          ...
                          {dayBookings.length > 0 && (
                            <div className="flex gap-1">
                              {Array.from(
                                new Set(dayBookings.map((b) => b.status)),
                              )
                                .slice(0, 3)
                                .map((status, idx) => (
                                  <Tooltip key={idx}>
                              ...
                              {dayBookings.length > 3 && (
                                <Tooltip>
                                  ...
                                  <TooltipContent>
                                    <div className="text-xs">
                                      {dayBookings.length} total bookings
                                    </div>
                                  </TooltipContent>
                                </Tooltip>
                              )}
                            ...
                            <Tooltip key={booking.id}>
                              <TooltipTrigger asChild>
                                <div
                                  ...
                                  onClick={(e) =>
                                    handleViewBookingDetails(e, booking)
                                  }
                                >
                                  <div
                                    className={`text-xs p-1 rounded border transition-all hover:shadow-md group-hover:shadow-emerald-200 ${getStatusColor(booking.status)} ${draggedBooking?.id === booking.id ? "opacity-50 scale-95" : ""}`}
                                  >
                                    <div className="font-medium truncate flex items-center gap-1">
                                      ...
                                      {booking.source === 'HOSTAWAY' && (
                                        <span className="text-[9px] bg-purple-600/80 text-white px-1 rounded font-bold">H</span>
                                      )}
                                      {booking.source === 'LOCAL' && (
                                        <span className="text-[9px] bg-emerald-600/80 text-white px-1 rounded font-bold">L</span>
                                      )}
                                      {booking.guestName}
                                    </div>
                                  ...
                                  <div className="invisible group-hover:visible flex gap-1 mt-1">
                                    <Button
                                      ...
                                      onClick={(e) =>
                                        handleViewGuestProfile(e, booking)
                                      }
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
            <h4 className="text-sm font-medium mb-2 text-emerald-800">
              Enterprise Features
            </h4>
            <div className="text-xs text-emerald-700 space-y-1">
              ...
              <div className="flex items-center gap-2">
                <Eye className="h-3 w-3" />
                <span>
                  Hover over bookings for detailed revenue & guest info
                </span>
              </div>
              <div className="flex items-center gap-2">
        ...
        <div className="mt-4 pt-4 border-t flex items-center gap-4 text-xs">
          ...
          <div className="flex items-center gap-2">
            <span className="bg-gray-500 text-white px-1.5 py-0.5 rounded font-bold text-[9px]">H</span>
            <span className="text-slate-600">Hostaway Booking</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-slate-600">🏢</span>
            <span className="text-slate-600">Hostaway Property</span>
          </div>
        </div>
      </CardContent>
