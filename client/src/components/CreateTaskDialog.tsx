...
interface CreateTaskDialogProps {
  ...
  defaultPropertyId?: number; // Auto-select property in form
}
...
export default function CreateTaskDialog({
  ...
  // Fetch authenticated user to get organization
  const { data: authUser } = useQuery<any>({
    queryKey: ["/api/auth/user"],
  });

  const organizationId = authUser?.organizationId || "default-org";

  // Fetch active staff members for assignment (single source of truth with Salaries module)
  const { data: staffMembers = [] } = useQuery<any[]>({
    queryKey: ["/api/staff-members", organizationId],
    queryFn: async () => {
      const response = await fetch(
        `/api/staff-members?organizationId=${organizationId}&status=active`,
      );
      if (!response.ok) {
  ...
  // Show all active staff - no user account required
  const staffArray = Array.isArray(staffMembers) ? staffMembers : [];
  console.log("staffArray from salaries module:", staffArray);
  ...
  // Auto-select property when defaultPropertyId is provided
  React.useEffect(() => {
    if (defaultPropertyId && isOpen) {
      form.setValue("propertyId", defaultPropertyId);
      console.log("Auto-selected property:", defaultPropertyId);
    }
  }, [defaultPropertyId, isOpen, form]);
  ...
  const createTaskMutation = useMutation({
    ...
    onSuccess: async (newTask) => {
      ...
      // Use centralized cache manager for cross-module invalidation
      // This clears fastCache and invalidates tasks + finance + dashboard
              ...
              render={({ field }) => (
                <FormItem>
                      ...
                      {staffArray.length > 0 ? (
                        staffArray.map((staff: any) => (
                          <SelectItem key={staff.id} value={`staff-${staff.id}`}>
                            ...
                            {staff.lastName} ({staff.position})
                          </SelectItem>
                        ))
                      ) : (
                        <div className="p-2 text-sm text-muted-foreground text-center">
                          No staff members found. Add staff in the Salaries
                          module first.
                        </div>
                      )}
                    </SelectContent>
                  ...
                  <FormDescription className="text-xs">
                    All active staff from the Salaries module
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
