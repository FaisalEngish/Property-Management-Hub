...
export async function apiRequest<T = any>(
  ...
  if (!response.ok) {
    // Extract error message: try message, error, or details fields
    const errorMsg = data?.message || data?.error || data?.details || `Request failed: ${response.status}`;
...
export const getQueryFn: <T>(options: {
  ...
  async ({ queryKey }) => {
    ...
    // Use full queryKey for caching to avoid issues with parameterized queries
    // (e.g., ['/api/currency/rates', 'THB'] vs ['/api/currency/rates', 'USD'])
    const cacheKey = queryKey.length > 1 ? JSON.stringify(queryKey) : urlKey;

    // Check instant data first for immediate response
    const instantData = getInstantData(urlKey);
    if (instantData) {
      console.log(`Instant data served: ${urlKey}`);
