...
// ========================================
// OWNER ONBOARDING SYSTEM + SMART DOCUMENT UPLOAD CENTER
// ========================================

// Owner Onboarding Process Tracking
export const ownerOnboardingProcesses = pgTable("owner_onboarding_processes", {
  id: serial("id").primaryKey(),
  organizationId: varchar("organization_id").references(() => organizations.id).notNull(),
  ownerId: varchar("owner_id").references(() => users.id).notNull(),
  propertyId: integer("property_id").references(() => properties.id),
  
  // Onboarding Steps (1-9 as per requirements)
  step1OwnerContactInfo: boolean("step1_owner_contact_info").default(false),
  step2PropertyBasics: boolean("step2_property_basics").default(false),
  step3LocationMapping: boolean("step3_location_mapping").default(false),
  step4PhotoUploads: boolean("step4_photo_uploads").default(false),
  step5PropertyDescription: boolean("step5_property_description").default(false),
  step6UtilityInfo: boolean("step6_utility_info").default(false),
  step7LegalDocuments: boolean("step7_legal_documents").default(false),
  step8SecurityAccess: boolean("step8_security_access").default(false),
  step9ServicesSetup: boolean("step9_services_setup").default(false),
  
  // Progress Tracking
  totalSteps: integer("total_steps").default(9),
  completedSteps: integer("completed_steps").default(0),
  progressPercentage: decimal("progress_percentage", { precision: 5, scale: 2 }).default("0.00"),
  currentStep: integer("current_step").default(1),
  
  // Deadline Management
  onboardingDeadline: timestamp("onboarding_deadline"),
  deadlineSetBy: varchar("deadline_set_by").references(() => users.id),
  isOverdue: boolean("is_overdue").default(false),
  daysPastDeadline: integer("days_past_deadline").default(0),
  
  // Status Tracking
  status: varchar("status").default("in_progress"), // not_started, in_progress, completed, overdue, on_hold
  startedAt: timestamp("started_at"),
  completedAt: timestamp("completed_at"),
  ...
  nextStepInstructions: text("next_step_instructions"),
  
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

// Document Upload Center (simplified version)
export const ownerDocuments = pgTable("owner_documents", {
  ...
  category: varchar("category").notNull(), // legal_documents, property_photos, utility_bills, etc.
  documentType: varchar("document_type"), // tabien_baan, blue_book, chanote, etc.
  description: text("description"),
...
// Owner Onboarding Step Details
export const onboardingStepDetails = pgTable("onboarding_step_details", {
  id: serial("id").primaryKey(),
  organizationId: varchar("organization_id").references(() => organizations.id).notNull(),
  onboardingId: integer("onboarding_id").references(() => ownerOnboardingProcesses.id).notNull(),
...
// Simplified Document Categories for Owner Onboarding
export const onboardingDocumentCategories = pgTable("onboarding_document_categories", {
  id: serial("id").primaryKey(),
  organizationId: varchar("organization_id").references(() => organizations.id).notNull(),
...
// ===== SCHEMA VALIDATIONS FOR OWNER ONBOARDING =====

export const insertOwnerOnboardingProcessSchema = createInsertSchema(ownerOnboardingProcesses).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertOwnerDocumentSchema = createInsertSchema(ownerDocuments).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const insertOnboardingStepDetailSchema = createInsertSchema(onboardingStepDetails).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type OwnerOnboardingProcess = typeof ownerOnboardingProcesses.$inferSelect;
export type InsertOwnerOnboardingProcess = z.infer<typeof insertOwnerOnboardingProcessSchema>;

export type OwnerDocument = typeof ownerDocuments.$inferSelect;
export type InsertOwnerDocument = z.infer<typeof insertOwnerDocumentSchema>;
