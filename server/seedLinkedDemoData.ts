...
export async function seedLinkedDemoData(): Promise<void> {
  ...
  try {
    ...
    const julyDate = new Date('2025-07-21'); // Property booking in July
    const julyEndDate = new Date('2025-07-26');

    const linkedBookings: InsertBooking[] = [
      // Past booking - Property (completed, should generate finance record)
      {
        organizationId: DEMO_ORG_ID,
        externalId: 'BK-2024-001',
        propertyId: createdProperties.find(p => p.name === 'Property')?.id || createdProperties[0].id,
      ...
      // Current booking - Property View (active)
      {
        organizationId: DEMO_ORG_ID,
        externalId: 'BK-2025-002',
        propertyId: createdProperties.find(p => p.name === 'Property View')?.id || createdProperties[1].id,
      ...
      // Future booking - Property
      {
        organizationId: DEMO_ORG_ID,
        externalId: 'BK-2025-003',
        propertyId: createdProperties.find(p => p.name === 'Property')?.id || createdProperties[3].id,
        guestName: 'Zhang Wei',
      ...
      // July booking - Property (the one mentioned in screenshot)
      {
        organizationId: DEMO_ORG_ID,
        externalId: 'BK-2025-004',
        propertyId: createdProperties.find(p => p.name === 'Property (Demo)')?.id || createdProperties[2].id,
